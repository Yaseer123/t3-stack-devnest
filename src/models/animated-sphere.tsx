...
useGLTF.preload("meet.glb");

export const AnimatedSphereModel = (props: JSX.IntrinsicElements["group"]) => {
  // Explicit casting to resolve ref error
  const group = useRef<Group>(null!); // Non-null assertion
  const { animations, scene } = useGLTF("meet.glb"); // Load meet.glb
  const { actions } = useAnimations(animations, scene);

  useEffect(() => {
    // Log the available animation names
    console.log("Available animations:", animations.map(anim => anim.name));

    const mainAction = actions?.[animations[0]?.name ?? ""]; // Use first animation if available
    if (mainAction) {
      mainAction.play();
